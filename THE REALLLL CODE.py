...
#    elementary_charge * 9,
#    elementary_charge * 9,
#] + [-elementary_charge] * 40 + [-elementary_charge * 2] * 2)
#predicted = 87.5
...
def graph(positions, energy_list,
    ...
    # ensure numpy arrays
    global partname
    path = rf"C:/Users/Anas Elwasifi/Desktop/Ppaer graphs/{partname}/{title}.html"
...
def main():
    ...
    while (Temp > 5):
        ...
        while (moves_since_change < threshold):
            ...
            if delta != 0:
                power = np.floor(np.log10(abs(delta)))
            else:
                power = 0.0

            delta_scaled = delta / (10**power)
            kT_scaled   = (Boltzmann * Temp) / (10**power)

            ratio = -delta_scaled / kT_scaled
            p_accept = np.exp(min(0, ratio))
            ...
            elif (np.random.uniform() < p_accept and delta != 0):
                best_potential = potential
                best_distr = particles.copy()
                moves_since_change = 0
